...
class MaterializationTransformer(CloningVisitor):
    ...
    def visit_select_query(self, node: ast.SelectQuery):
        # Process CTEs first with proper context tracking
        new_ctes: Optional[dict[str, ast.CTE]] = None
        if node.ctes:
            new_ctes = {}
            for cte_name, cte in node.ctes.items():
                prev_cte = self._current_cte_name
        ...
        # Now visit the select query itself (without re-visiting CTEs)
        # Temporarily remove CTEs to prevent super() from visiting them again
        ...
        # Determine which variables apply to the current context
        ...
        if vars_for_context:
            ...
            # Add GROUP BY only if query already has GROUP BY (or for top-level vars always)
            if new_node.group_by is not None or self._current_cte_name is None:
                self._add_group_by(new_node, vars_for_context)

            # Remove variable WHERE clauses
            if new_node.where:
                new_node.where = self._remove_variable_from_where(new_node.where)

        # Top-level query: also add passthrough columns for CTE variables
        if self._current_cte_name is None and cte_vars:
            passthrough_additions = [ast.Field(chain=[var.code_name]) for var in cte_vars]
            if new_node.select:
                new_node.select = [*list(new_node.select), *passthrough_additions]
            else:
                new_node.select = passthrough_additions

            # Add GROUP BY if query aggregates (explicit GROUP BY or aggregate functions in SELECT)
            if new_node.group_by is not None or self._has_aggregate_functions(new_node):
                self._add_group_by(new_node, cte_vars, use_field_ref=True)
