...
class TestPostgresPrinter(BaseTest):
    ...
    @parameterized.expand(
        [
            ...
            ("toDayOfWeek", "toDayOfWeek(now())", "EXTRACT(DOW FROM NOW())"),
            ...
            ("toStartOfYear", "toStartOfYear(now())", "CAST(DATE_TRUNC('year', NOW()) AS DATE)"),
            ("toStartOfQuarter", "toStartOfQuarter(now())", "CAST(DATE_TRUNC('quarter', NOW()) AS DATE)"),
            ("toStartOfMonth", "toStartOfMonth(now())", "CAST(DATE_TRUNC('month', NOW()) AS DATE)"),
            ("toStartOfWeek", "toStartOfWeek(now())", "CAST(DATE_TRUNC('week', NOW()) AS DATE)"),
            ("toMonday", "toMonday(now())", "CAST(DATE_TRUNC('week', NOW()) AS DATE)"),
            ("toStartOfDay", "toStartOfDay(now())", "DATE_TRUNC('day', NOW())"),
            ("toStartOfHour", "toStartOfHour(now())", "DATE_TRUNC('hour', NOW())"),
            ("toStartOfMinute", "toStartOfMinute(now())", "DATE_TRUNC('minute', NOW())"),
            ("toStartOfSecond", "toStartOfSecond(now())", "DATE_TRUNC('second', NOW())"),
            (
                "toLastDayOfMonth",
            ...
            (
                ...
                "DATE_TRUNC('minute', NOW()) - CAST(EXTRACT(MINUTE FROM NOW()) AS INTEGER) % 5 * INTERVAL '1 minute'",
            ),
            (
                "toStartOfTenMinutes",
                "toStartOfTenMinutes(now())",
                "DATE_TRUNC('minute', NOW()) - CAST(EXTRACT(MINUTE FROM NOW()) AS INTEGER) % 10 * INTERVAL '1 minute'",
            ),
            (
                "toStartOfFifteenMinutes",
                "toStartOfFifteenMinutes(now())",
                "DATE_TRUNC('minute', NOW()) - CAST(EXTRACT(MINUTE FROM NOW()) AS INTEGER) % 15 * INTERVAL '1 minute'",
            ),
            # ISO year start
            ("toStartOfISOYear", "toStartOfISOYear(now())", "CAST(DATE_TRUNC('year', NOW()) AS DATE)"),
            # Last day of week
            (
                "toLastDayOfWeek",
                "toLastDayOfWeek(now())",
    ...
    @parameterized.expand(
        [
            ...
            ("anyIf", "anyIf(1, 1)", "any(1) FILTER (WHERE 1)"),
    ...
    def test_standard_sql_functions_pass_through(self, _name: str, expr: str):
        # Should not raise — these are in the passthrough set
