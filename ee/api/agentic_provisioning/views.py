...
def provisioning_resource_detail(request: Request, resource_id: str) -> Response:
    return _resolve_resource(request, resource_id)
...
def provisioning_rotate_credentials(request: Request, resource_id: str) -> Response:
    # We intentionally return the current API key without regenerating it.
    # The key is embedded in the customer's SDK config — rotating it would
    # break their integration. Same approach as the Vercel integration.
    return _resolve_resource(request, resource_id)
...
def _resolve_resource(request: Request, resource_id: str) -> Response:
    auth_error, user, access_token = _authenticate_bearer(request)
    if auth_error:
        return auth_error
    assert access_token is not None
    ...
    try:
        ...
        logger.warning("stripe_app.resource_resolve.team_not_found", team_id=team_id)
        capture_exception(Exception("Stripe APP resource resolve: team not found"))
        return Response(
